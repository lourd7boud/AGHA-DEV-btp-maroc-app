...
const ALLOWED_COLUMNS: Record<string, string[]> = {
  ...
  metres: [
    ...
    'designation_bordereau', 'unite', 'lignes', 'total_partiel', 'total_cumule',
    'quantite_bordereau', 'pourcentage_realisation'
  ],
  decompts: [
...
async function applyUpdate(
  ...
  try {
    const cleanId = cleanEntityId(entityId);
    const data = preparePayloadForDb(tableName, payload, '');
    ...
    if (result.rowCount === 0) {
      logger.warn(`UPDATE: No rows affected for ${tableName}/${cleanId}`);
    } else {
        ...
        try {
          switch (type) {
            case 'CREATE':
              ...
              break;
            case 'UPDATE':
              applyResult = await applyUpdate(client, tableName, entityId, data, opId);
              break;
            case 'DELETE':
  ...
  try {
    ...
    if (sinceSeq > 0) {
      ...
      if (lastSyncTimestamp === 0 || lastSyncTimestamp < 1000000000000) {
        ...
        logger.info(`[${requestId}] Full sync: returning ${operations.length} operations`);
...
export const resolveConflictV2 = async (
  ...
  try {
    ...
    const opId = uuidv4();
    await applyUpdate(client, tableName, conflict.entity_id, dataToApply, opId);
